...
console.log("[auto-drama-server] preparing Next.js app...", { dev, dir: __dirname, port: PORT });

const prepareStarted = Date.now();
const prepareWarnTimer = setTimeout(() => {
  // eslint-disable-next-line no-console
  console.warn("[auto-drama-server] Next.js prepare() taking >10s (possible hang).");
}, 10_000);
...
void app
  ...
  .then(() => {
    ...
    // eslint-disable-next-line no-console
    console.log("[auto-drama-server] Next.js prepared", { ms: Date.now() - prepareStarted });
  ...
  server.listen(PORT, () => {
    // eslint-disable-next-line no-console
    console.log(`[auto-drama-server] Next + Socket.io on http://localhost:${PORT}`);
  });
  })
  .catch((e) => {
    clearTimeout(prepareWarnTimer);
    // eslint-disable-next-line no-console
    console.error("[auto-drama-server] Next.js prepare failed", e);
    process.exit(1);
  });
