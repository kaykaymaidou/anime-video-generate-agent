...
import { resolveVideoTaskBackend, resolveVideoTaskGatewayBase } from "@/lib/video-task-config";
...
export type PythonEvent =
  ...
  | {
      ...
      shotId?: string;
      taskId?: string;
      /** 网关 HTTP 轮询进度（与 Socket 应用层 heartbeat 不同） */
      kind?: "gateway-poll";
      backend?: "nest" | "py";
    }
...
export class PythonBridgeService {
  ...
  constructor(opts: PythonBridgeOptions) {
    ...
    this.gatewayUrl = resolveVideoTaskGatewayBase();
  ...
  runTask(task: unknown) {
    ...
    const script = path.join(this.repoRootDir, "services", "anime-video-generate-agent-ai", "src", "main.py");
  ...
  private async runTaskViaGateway(task: unknown, taskId?: string, shotId?: string) {
    ...
    const parseJsonBody = async (res: Response): Promise<Record<string, unknown>> => {
      ...
      if (!trimmed) return {};
      const tryParse = (): Record<string, unknown> => JSON.parse(trimmed) as Record<string, unknown>;
      if (ct.includes("json")) {
        try {
          return tryParse();
        } catch {
          throw new Error(trimmed.length > 600 ? `${trimmed.slice(0, 600)}…` : trimmed || `HTTP ${res.status}`);
        }
      }
    ...
    try {
      ...
      const data = (await parseJsonBody(res)) as { ark_task_id?: string };
      ...
      while (Date.now() < deadline) {
        ...
        const snap = (await parseJsonBody(gr)) as Record<string, unknown>;
        ...
        emit({
          ...
          kind: "gateway-poll",
          backend: resolveVideoTaskBackend() === "nest-http" ? "nest" : "py"
        });
      }
