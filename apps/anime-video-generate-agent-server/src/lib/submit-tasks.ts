...
import { describeVideoTaskBackend } from "@/lib/video-task-config";
...
export function handleTaskSubmit(
  ...
  // 里程碑：任务已被服务端接收
  const backendLabel = describeVideoTaskBackend();
  emitProgress({
    event: "log",
    taskId,
    message: `server accepted task; backend=${backendLabel}`
  } as any);
  opts.io.emit("progress-update", { event: "log", taskId, message: `server accepted task; backend=${backendLabel}` });
  // 默认也打印关键链路（避免“啥也没有输出”）
  // eslint-disable-next-line no-console
  ...
  for (const s of parsed.data.shots) {
    const runMsg =
      backendLabel === "py-stdin"
        ? "spawning python worker for shot"
        : `submitting shot via ${backendLabel}`;
    emitProgress({ event: "log", taskId, shotId: s.id, message: runMsg } as any);
    opts.io.emit("progress-update", { event: "log", taskId, shotId: s.id, message: runMsg });
    // eslint-disable-next-line no-console
    console.log("[pipeline] run shot", { taskId, shotId: s.id, backend: backendLabel, modelType: s.modelType, duration: s.duration });
    if (pipelineDebug) {
      // eslint-disable-next-line no-console
      console.log("[pipeline] run shot(debug)", { taskId, shotId: s.id, backend: backendLabel, modelType: s.modelType, duration: s.duration });
    }
    bridge.runTask({
      taskId,
      shotId: s.id,
