...
def build_content(
    *,
    ...
    - fps 仅保留函数参数以兼容调用方；当前 volcenginesdkarkruntime 的 tasks.create 不支持 fps 字段，不会写入 body。
    """
    ...
    # Ark Python SDK Tasks.create() 不接受 fps；官方 REST 若支持也需换接入方式，此处不把 fps 写入 body。
    _ = fps  # 保留函数参数供调用方/文档对齐，避免破坏签名
...
def create_task(*, client: Ark, body: Dict[str, Any]) -> str:
    ...
    # 与 SDK 版本对齐：多余字段会导致 TypeError（例如 fps）
    for drop in ("fps",):
